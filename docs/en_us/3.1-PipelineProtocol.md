...
- **The entry node participates in recognition normally.** Do **not** write it as `DirectHit` — otherwise the sub-pipeline hits on the very first frame and skips the real probes in `entry.next`.
