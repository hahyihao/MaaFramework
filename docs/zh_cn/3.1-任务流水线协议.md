...
4. **后继处理**
   ...
   - 若某个子节点识别命中且动作成功，进入该节点的 `next` 列表，重复上述检测流程
   - 若某个子节点识别命中但动作失败，进入该节点的 `on_error` 列表
   - 若当前节点的 `next` 列表识别超时，进入当前节点的 `on_error` 列表
...
```mermaid
    graph LR
        ...
        E -->|8.1.超时| ZA[当前节点失败，进入 on_error]
        E -->|8.2.未超时| A
        A -->|9.再次识别| B
        B -->|10.识别成功，执行动作| F[点击]
```
...
- 识别范围由 `roi` 和 `roi_offset` 共同决定。
- `box`：识别命中后返回的匹配区域，表示“识别到了哪里”。
- `target`：动作执行目标区域，用于决定“点哪里 / 滑哪里 / 在哪里滚动”。
- 动作目标由 `target` 和 `target_offset` 共同决定。
- `target` 默认值为 `true`，即使用当前识别得到的 `box` 作为 `target`；也可以由用户显式指定固定点、固定区域或其他节点。

典型流程是：先在 `roi`（含 `roi_offset`）内进行识别，得到 `box`，再计算 `target`（含 `target_offset`）并执行动作。
...
- `recognition` : *string*  
    ...
    可选的值：[`DirectHit`](#directhit) | [`TemplateMatch`](#templatematch) | [`FeatureMatch`](#featurematch) | [`ColorMatch`](#colormatch) | [`OCR`](#ocr) | [`NeuralNetworkClassify`](#neuralnetworkclassify) | [`NeuralNetworkDetect`](#neuralnetworkdetect) | [`And`](#and) | [`Or`](#or) | [`Custom`](#custom) | [`SubPipeline`](#subpipeline)  
    详见 [算法类型](#算法类型)。

- `action`: *string*  
    执行的动作。可选，默认 [`DoNothing`](#donothing) 。  
    可选的值：[`DoNothing`](#donothing) | [`Click`](#click) | [`LongPress`](#longpress) | [`Swipe`](#swipe) | [`MultiSwipe`](#multiswipe) | [`Scroll`](#scroll) | [`ClickKey`](#clickkey) | [`LongPressKey`](#longpresskey) | [`InputText`](#inputtext) | [`StartApp`](#startapp) | [`StopApp`](#stopapp) | [`StopTask`](#stoptask) | [`Command`](#command) | [`Shell`](#shell) | [`Screencap`](#screencap) | [`Custom`](#custom-1)  
    详见 [动作类型](#动作类型)。
...
- `task_mode`: *string* **💡 v5.x**  
    任务执行模式。可选，默认 `"state_machine"`。**仅 entry 节点该字段起作用**，子节点声明会被忽略。  
    取值：

  - `"state_machine"`：传统状态机。从 entry 出发沿 `next` 走，链走完任务结束。
  - `"loop_scan"`：循环扫描模式。框架持续扫描整条 `next` 链，命中节点执行 action 后等待 `cycle_delay` 进入下一轮；全部未命中时跳到 `[Fallback]` 节点；任务永不主动结束（需外部 stop）。

- `cycle_delay`: *uint* | *[uint, uint]* **💡 v5.x**  
    `loop_scan` 模式下每轮循环结束的等待时长，单位毫秒。可选，默认 1000 。  
    支持两种写法：
...
- `sub_pipeline`: *string* **💡 v5.x**  
    命中本节点后递归进入的子 pipeline 入口节点名（**仅 `task_mode: "loop_scan"` 下生效**）。可选，默认空。  
    子层执行**单次穿透**：

  - 扫一帧子层 chain
  - 命中节点执行 action（若该节点也声明了 `sub_pipeline`，继续递归）
  - 全部未命中 → 跑子层兜底 `[Fallback]`
  - 函数返回 → 回到上一层 → 最终回到顶层 main 等待 `cycle_delay`

    最大递归深度 8 层，超过即放弃当前递归层并打日志。  
    引用支持以下形式：
  - 裸名：如 `"sub_entry"` —— 走全局唯一回退查找
  - FQN：如 `"battle/fight::entry"` —— 直接查找（当多文件场景下节点同名时使用）

- `timeout`: *int*  
    当前节点 `next` 列表识别的超时时间，毫秒。可选，默认 20 * 1000（20 秒）。  
    设置为 -1 表示无限等待，永不超时。**💡 v5.5**  
    可近似理解为 `while(!hit && !timeout) { foreach(next); sleep_until(rate_limit); }` 。

- `on_error` : *string* | *NodeAttr* | *list<string | NodeAttr, >*  
    当**本节点**内的 `next` 列表中没有任何节点命中并且循环识别超时，或动作执行失败后，接下来会执行该列表中的节点（参见[流程图](#节点流程)）。可选，默认空。  
    💡 v5.1 版本起新增支持带属性节点、 NodeAttr 形式对象，或两者结合的异质数组。详情请参考 [节点属性](#节点属性)。
...
- `anchor`: *string* | *list<string, >* | *object* **💡 v5.1**  
  ...
  - **对象** **💡 v5.7**：`"anchor": {"A": "TargetNode", "B": ""}` - 将锚点 A 设置为 TargetNode，锚点 B 清除（空字符串表示清除该锚点）
    ...
    在 `next` 或 `on_error` 中可通过 `[Anchor]` 属性引用该锚点，运行时会解析为最后设置该锚点的节点。如果引用的锚点未设置或已被清除，该节点将被跳过（不会执行）。  
    详见 [节点属性](#节点属性)。
...
- `repeat`: *uint* **💡 v5.3**  
    ...
    重复过程中单次动作失败不会提前中止，最终结果以最后一次动作为准。
...
- `focus`: *object*  
    ...
    每个消息类型的值支持 `string`（旧行为，等价于 `display: "log"`）或 `object`（新行为，可指定 `display`）。  
    详见 [节点通知](#节点通知)。
...
- `roi`: *array<int, 4>* | *string*  
  ...
  - *string*: 填写节点名，在之前执行过的某节点识别到的目标范围内识别。也支持 `[Anchor]锚点名` 格式引用锚点对应的节点 **💡 v5.9**。若引用的前置节点或锚点识别结果为空，则视为识别失败。  
...
- `template`: *string* | *list<string, >*  
    ...
    所使用的图片需要是无损原图缩放到 720p 后的裁剪。请参考 [图片文件](1.1-快速开始.md#图片文件)。  
...
- `method`: *int*  
    ...
    *10001 为 TM_SQDIFF_NORMED 的反转版本，分数越高越匹配（与原版相反）。* **💡 v5.3**

    | method | 算法名称 | 速度 | 颜色敏感度 | 光照鲁棒性 | 适用场景 |
    | ------ | ------- | ---- | --------- | --------- | ------- |
    | 10001 | TM_SQDIFF_NORMED (Inverted) **💡 v5.3** | 快 | 敏感 | 差 | 精确匹配，阈值易设定 |
...
- `all_of`: *list\<string | object\>*  
    ...
    列表元素可以是：
  - 字符串 **💡 v5.7**：节点名称引用，运行时使用该节点的识别算法和参数
...
```jsonc
{
    ...
    "ConfirmDialog": {
        "recognition": "And",
        "all_of": [
            "CheckIcon",   // 引用 CheckIcon 节点的识别参数
            "CheckText"    // 引用 CheckText 节点的识别参数
        ]
    }
}
```
...
```jsonc
{
    ...
    "AnyButton": {
        "recognition": "Or",
        "any_of": [
            "ConfirmButton",  // 引用 ConfirmButton 节点的识别参数
            "CancelButton"    // 引用 CancelButton 节点的识别参数
        ]
    }
}
```
...
#### U2 — 集成 openatx/uiautomator2 的现成适配器（Python only）

MaaFramework 自带一个 `CustomRecognition` 适配器,把 reco 阶段直接转发给 [openatx/uiautomator2](https://github.com/openatx/uiautomator2),用 uiautomator2 的原生 selector 找控件,拿 bounds 当 box 上浮。

**安装与初始化**:

```bash
pip install MaaFw[u2]              # 装 maa + uiautomator2
...
device = u2.connect()  # 或 u2.connect("serial") / u2.connect_usb()
...
`custom_recognition_param` 字段直接透传到 uiautomator2 的 selector(命名沿用 uiautomator2 风格,camelCase):

- 字符串过滤:`resourceId` / `text` / `textContains` / `textMatches` / `className` / `description` / `descriptionContains` / `packageName` 等
- 布尔过滤:`clickable` / `enabled` / `focusable` / `focused` / `checkable` / `checked` / `selected` / `scrollable` / `longClickable`
- 多匹配定位:`instance`(取第 N 个)
- XPath 表达式:`xpath`(走 `d.xpath(...).get()`)

字段命名规则、匹配语义、可用值全部跟 [uiautomator2 文档](https://github.com/openatx/uiautomator2#user-content-table-of-contents) 保持一致。

> **仅 Python binding 可用**(NodeJS 端不支持)。设备需要预先 `python -m uiautomator2 init` 推送 atx-agent。

### `SubPipeline`

把本节点的**识别阶段委托**给一个子流水线（"识别代理"语义）。

- 子流水线任一节点命中 → 本节点视为命中（命中节点的 `box` 上浮作为本节点 reco 结果的 box）；
- 子流水线全部 miss → 本节点视为 miss（外层继续尝试 `next` 里的下一个候选）。

子流水线按和 `main` 完全相同的方式写（pipeline 节点的全部字段都可用），唯一约束是 **single-pass**（不循环）。

该算法属性需额外字段：

- `recognition_pipeline`: *string*  
    指向一个 entry 节点名（同文件裸名 / FQN 全名都支持，按现有 `sub_pipeline` / `fallback_node` 字段的规则解析）。必选。
...
`main` 不做图色识别，只按顺序"先试主页面 → 再试弹窗"。识别逻辑下放到对应的子文件里。

#### ⚠️ 注意事项

- **entry 节点本身不参与识别**，只作为调度入口 —— 引擎扫描的是 `entry.next` 列表。一般写 `recognition: "DirectHit"` 即可（不会真的"命中"，因为该节点不会被扫到）。
- 子流水线里的 `[Fallback]` 节点和 entry 上的 `fallback_node` 字段在 `SubPipeline` 语境下**被忽略**（fallback 必命中会破坏"探针"语义）。
- 子流水线节点上声明的 `task_mode: "loop_scan"` 在 `SubPipeline` 语境下**被忽略**（强制 single-pass）。
- 最大递归深度 **8 层**。超过该深度直接将本节点视为 miss 并记 warn 日志。

#### 与 `sub_pipeline` 字段的区别

| 维度 | `recognition: "SubPipeline"` | 节点顶层 `sub_pipeline: "..."` 字段 |
|------|------------------------------|-----------------------------------|
| 触发阶段 | reco 阶段 | action 之后 |
| 影响父节点 hit/miss | 是 | 否（父已经命中） |
| 用途 | 委托识别（探针组） | 命中后调子例程 |

两者**可以同时声明**：先用 `SubPipeline` 确认进入了主页面，命中后再跑主页面子例程。
...
- `target`: *true* | *string* | *array<int, 2>* | *array<int, 4>*  
  ...
  - *array<int, 2>*: 固定坐标点 [x, y] **💡 v5.0**。支持负数 **💡 v5.6**，表示从右/下边缘计算。
  - *array<int, 4>*: 固定坐标区域 [x, y, w, h]，在矩形内随机选点（中心概率高）。支持负数 **💡 v5.6**，含义同 `roi`。
...
- `contact`: *uint* **💡 v5.0**  
  ...
  - Adb 控制器：表示手指编号（0 为第一根手指，1 为第二根手指，以此类推，最大不能超过 10）
  - Win32 控制器：表示鼠标按键编号（0 为左键，1 为右键，2 为中键，3 为 XBUTTON1，4 为 XBUTTON2）
  - MacOS 控制器：表示鼠标按键编号（0 为左键，1 为右键，2 为中键）
...
- `swipes`: *list<object,>*  
  ...
  - `contact`: *uint* **💡 v5.0**  
    触点编号，用于区分不同的触控点。可选，默认使用该滑动在数组中的索引作为触点编号（从 0 开始）。
    - Adb 控制器：表示手指编号（0 为第一根手指，1 为第二根手指，以此类推，最大不能超过 10）
    - Win32 控制器：表示鼠标按键编号（0 为左键，1 为右键，2 为中键，3 为 XBUTTON1，4 为 XBUTTON2）
    - MacOS 控制器：表示鼠标按键编号（0 为左键，1 为右键，2 为中键）
...
> [!NOTE]
>
> - Win32 和 macOS 控制器，以及实现了 `scroll` 的自定义控制器支持滚动操作。
...
- `key`: *int* | *list<int, >*  
  ...
  - [MacOS 控制器](https://github.com/phracker/MacOSX-SDKs/blob/master/MacOSX10.6.sdk/System/Library/Frameworks/Carbon.framework/Versions/A/Frameworks/HIToolbox.framework/Versions/A/Headers/Events.h#L183-L329)
  - [WlRoots 控制器](https://git.kernel.org/pub/scm/linux/kernel/git/torvalds/linux.git/tree/include/uapi/linux/input-event-codes.h)
...
停止当前任务链（MaaTaskerPostTask 传入的单个任务链），中断后续识别循环。
...
保存当前截图到文件。截图保存在 `log_dir/screencap/` 目录下。
...
> 截图文件路径可通过 `MaaTaskerGetActionDetail` 在动作详情的 `filepath` 字段中获取。
...
{
    "A": {
        "anchor": "X",  // 将锚点 X 设置为当前节点 A
        "next": ["C"]
    },
    "B": {
        "anchor": "X",  // 将锚点 X 重新设置为当前节点 B（覆盖之前的设置）
        "next": ["C"]
    },
    "C": {
        "next": [
            "D",
            "[Anchor]X"  // 引用锚点 X，会跳转到最后设置该锚点的节点（B）
        ]
    }
}
```
...
使用对象格式，可以将锚点设置为其他节点，或清除锚点：

```jsonc
{
    "A": {
        "anchor": {"X": "TargetNode"},  // 将锚点 X 设置为 TargetNode
        "next": ["B"]
    },
    "B": {
        "next": ["[Anchor]X"]  // 引用锚点 X，会跳转到 TargetNode
    },
    "TargetNode": {
        // ...
    }
...
```jsonc
{
    "A": {
        "anchor": {
            "X": "TargetX",  // 锚点 X -> TargetX
            "Y": "",         // 清除锚点 Y（空字符串表示清除该锚点）
            "Z": "TargetZ"   // 锚点 Z -> TargetZ
        },
        "next": ["B"]
    }
}
...
- 基础用法：适用于需要动态引用节点的场景，例如多个节点可能处理同一类事件，后续需要回到"最后处理该事件的节点"继续执行。
- 高级用法：允许将锚点映射到特定的目标节点，或通过空字符串清除锚点，提供了更灵活的流程控制能力，类似于给 pipeline 引入了变量机制。

#### `is_fallback` / `[Fallback]` **💡 v5.x**

**类型**: *boolean*  
**默认值**: `false`  
**说明**: 标记任务全局兜底节点。仅在 entry 节点的 `next` 列表中可声明，最多一个。  
**仅在 `task_mode: "loop_scan"` 下生效**。

当 entry 节点的 next 链一帧扫描全部未命中时，框架会跳到 `[Fallback]` 标记的节点并执行其 action（任意识别类型，常用 `DirectHit`），随后回到循环顶部等待 `cycle_delay` 后开始下一轮。

`[Fallback]` 节点本身**不参与**主链识别（解析期已被从 `next` 列表中剥离）。
...
> [!NOTE]
>
> 节点通知依赖上层实现，非 MaaFramework 原生支持。  
> 请参考您所使用的通用 UI 文档或代码以确认是否支持该功能。

通过配置 `focus` 字段，可以让 UI 在节点执行的特定阶段向用户展示自定义消息。请参考 [PI 协议](3.3-ProjectInterfaceV2协议.md#消息模板机制)。
